...
- ✅ Consultas complejas → asesora + pausa (_is_complex_query + QUESTION_HANDOFF): links (http/www), texto de 10+ palabras, preguntas con ? de 3+ palabras, o mención de sede/sucursal. Aplica en menú y pasos de elección (opt1_choice, ppf_protect, clarify, antiatraco_choice, handoff_agendar); NO en pasos de datos (vehicle, arch_*) para no romper medidas largas. Ejemplos del usuario verificados en vivo (5/5). Prioriza sobre el enrutador de keywords. Tests: TestComplexQueryHandoff (9 casos). 86/86 pytest OK.
